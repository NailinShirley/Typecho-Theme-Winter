      ...
      <div class="vh">
        <div class="vhead">
          <span class="vnick"><?php $comments->author(); ?></span>
            <span class="vsys"><?php echo getBrowser($comments->agent); ?></span>
            <span class="vsys"><?php echo getOs($comments->agent); ?></span>
        </div>
        <div class="vmeta">
          <span class="vtime"><?php $comments->date('Y-m-d') ?> | <?php $comments->date('H:i') ?></span>
  ...
  <form method="post" action="<?php $this->commentUrl() ?>" id="comment-form" onsubmit="return false">
    <?php if ($this->user->hasLogin()): ?>
      ...
      <div class="vwrap">
        ...
        <div class="vedit">
          <textarea name="text" id="veditor" class="veditor vinput" placeholder="发表动态"><?php $this->remember('text', false); ?></textarea>
        </div>
        <div class="vcontrol">
          ...
          <div class="col col-80 text-right">
            <button type="submit" title="Cmd|Ctrl+Enter" class="vsubmit vbtn" id="misubmit" onclick="javascript:location.reload();">发表</button>
  ...
  <center>
    <div class="vinfo" style="display:block;">
      <div class="vcount col">
        <span class="vnum"><?php $this->commentsNum('%d'); ?> 条动态</span>
      </div>
    </div>
    </center>
  <?php else: ?>
    <div class="vempty" style="display:block;">留下你的碎片吧</div>
  <?php endif; ?>
